    ...
    public IActionResult GetHealthTrendData([FromBody] DashboardQueryRequest request)
    {
        try
        {
            ...
            var healthData = _context.HealthMonitorings
                .Where(h => h.ElderlyId == request.ElderlyId 
                       && h.MonitoringDate >= request.StartDate 
                       && h.MonitoringDate <= request.EndDate)
                .OrderBy(h => h.MonitoringDate)
                .Select(h => new {
                    Date = h.MonitoringDate,
