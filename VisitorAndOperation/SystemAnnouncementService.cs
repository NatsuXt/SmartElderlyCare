...
public class SystemAnnouncementService
{
    ...
    /// <param name="commenterType">评论者身份类型（家属/员工/老人）</param>
    /// <returns>操作结果，true为成功，false为失败</returns>
    public bool AddComment(int announcementId, string comment, int commenterId, string commenterType)
    {
        var ann = _context.SystemAnnouncements.Find(announcementId);
        if (ann == null) return false;
        
        // 在原有评论基础上追加新评论，包含评论者信息
        var timestamp = DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss");
        var newComment = $"[{timestamp}] [{commenterType}ID:{commenterId}] {comment}";
        
        if (string.IsNullOrEmpty(ann.comments))
        {
            ann.comments = newComment;
        }
        else
        {
            ann.comments += $"\n{newComment}";
