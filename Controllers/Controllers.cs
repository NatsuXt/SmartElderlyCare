...
namespace Staff_Info.Controllers
{
    ...
    public class StaffInfoController : ControllerBase
    {
        ...
        [HttpPut("staff-infos/{id}")]
        public async Task<IActionResult> UpdateStaffInfo(decimal id, [FromBody] PostDeleteStaffInfoDto dto)
        {
            if (id != dto.StaffId) 
            ...
            staff.CONTACT_PHONE = dto.ContactPhone ?? staff.CONTACT_PHONE;
            staff.SKILL_LEVEL = dto.SkillLevel ?? staff.SKILL_LEVEL;
            staff.WORK_SCHEDULE = dto.WorkSchedule ?? staff.WORK_SCHEDULE;
            staff.EMAIL = dto.Email ?? staff.EMAIL;
            staff.HIRE_DATE = dto.HireDate ?? staff.HIRE_DATE;
            ...
            {
                return StatusCode(500, $"更新员工信息时发生错误: {ex.Message}");
            }
        }
        ...
        [HttpGet("nursing-plans/elderly/{elderlyId}")]
        public async Task<ActionResult<IEnumerable<NursingPlanDto>>> GetNursingPlansByElderlyId(decimal elderlyId)
        {
            var plans = await _context.NursingPlans
                .Include(np => np.STAFF)
                .Include(np => np.ELDERLY)
                .Where(np => np.ELDERLY_ID == elderlyId)
                .Select(np => new NursingPlanDto
                {
                    PlanId = np.PLAN_ID,
                    ElderlyId = np.ELDERLY_ID,
        ...
        // 获取所有记录
        [HttpGet("disinfection/records")]
        public async Task<ActionResult<IEnumerable<DisinfectionRecordDto>>> GetAllDisinfectionRecords()
        {
            var records = await _context.DisinfectionRecords
                .Select(r => new DisinfectionRecordDto
                {
                    DisinfectionId = r.DISINFECTION_ID,
                    Area = r.AREA,
                    DisinfectionTime = r.DISINFECTION_TIME,
                    StaffId = r.STAFF_ID,
                    Methods = r.METHODS,
                })
                .ToListAsync();
